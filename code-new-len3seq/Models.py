...
class PILES(nn.Module):
    def __init__(self, args):
        ...
        self.linear1 = nn.Linear(args.H_dim, args.marker_num) # 求lambda用的线性层
    ...
    def impute(self, H, F, front): # H为前一个event，F为后一个event，front为前一个序列已有的event（非后来插补的）

        emb_H = self.embedding(H[-1])
        emb_F = self.embedding(F[0])

        emb_front = self.embedding(front)
        emb = torch.stack((emb_front, emb_H, emb_F), dim=0)


        x_hat = self.transformer(emb.view(1, emb.shape[0], -1))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_seq_len', type=int, default=3, help='maximum input size')
